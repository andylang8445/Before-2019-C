...
int max_sum = 0;
int route_count = 0;
bool accept_vertical = true;
bool accept_horizental = true;
int f(int left_x, int left_y, int current_x, int current_y, int sum,int pass_count)
{
	passed[pass_count] = map[current_y][current_x];
	for(int i=0;i<pass_count;i++)
		if()
	if (left_x == 0 && left_y == 0||current_x==M&&current_y==N)
	{
		if (max_sum < sum)
			max_sum = sum;
		route_count++;
	...
	if (left_x > 0)
		f(left_x - 1, left_y, current_x + 1, current_y, sum + map[current_y][current_x],pass_count+1);
	if (left_y > 0)
		f(left_x, left_y - 1, current_x, current_y + 1, sum + map[current_y][current_x],pass_count+1);
	return 1;
}
void pre_set()
{
...
int main()
{
	...
	int left_x, left_y;
	scanf_s("%d", &T);
	for (int run_count = 1; run_count <= T; run_count++)
	{
		input();
		left_x = N - 1;
		left_y = M - 1;
		f(left_x, left_y, cx, cy, 0, 0);
		printf("#%d %d %d\n", run_count, route_count, max_sum);
	}
	_getch();
}*/
